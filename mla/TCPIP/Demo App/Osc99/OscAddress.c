 ...
 * found then the result of MatchExpression is returned.
 *
 ...
 * "/abc[!d-hijk[p-l]qrst" will match the OSC address "/abcXqrst" provided that
 ...
 * @code
 * OscMessage oscMessage;
 * OscMessageInitialise(oscMessage.oscAddressPattern, "/example/oscAddress/pattern");
 ...
 *     char string[16];
 *     OscAddressGetPartAtIndex(&oscMessage, i, string, sizeof (string));
 *     printf("%s ", string);
 * }
 * @endcode
 *
 ...
 */
int OscAddressGetPartAtIndex(const char* oscAddressPattern, const int index, char* const destination, const size_t destinationSize) {
    ...
    while (partCount < (index + 1)) {
        ...
        if (*oscAddressPattern == '\0') {
            return 1; // error: not enough parts in message oscAddress
        }
    }
